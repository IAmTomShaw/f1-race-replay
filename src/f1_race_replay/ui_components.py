...
class LegendComponent(BaseComponent):
    def __init__(self, x: int = 20, y: int = 150):
        ...
        icons_folder = IMAGES_FOLDER / "controls"
        if icons_folder.is_dir():
            for child in icons_folder.iterdir():
                if child.suffix in (".png", ".jpg", ".jpeg"):
                    self._control_icons_textures[child.stem] = arcade.load_texture(child.as_posix())
        self.lines = [
            "Controls:",
            "[SPACE]  Pause/Resume",
            "[←/→]    Rewind / FastForward",
...
class WeatherComponent(BaseComponent):
    def __init__(self, left=20, width=280, height=130, top_offset=170):
        ...
        weather_folder = IMAGES_FOLDER / "weather"
        if weather_folder.is_dir():
            for child in weather_folder.iterdir():
                if child.suffix in (".png", ".jpg", ".jpeg"):
                    self._weather_icon_textures[child.stem] = arcade.load_texture(child.as_posix())
...
class LeaderboardComponent(BaseComponent):
    def __init__(self, x: int, right_margin: int = 260, width: int = 240):
        ...
        tyres_folder = IMAGES_FOLDER / "tyres"
        if tyres_folder.is_dir():
            for child in tyres_folder.iterdir():
                if child.suffix in (".png", ".jpg", ".jpeg"):
                    self._tyre_textures[child.stem] = arcade.load_texture(child.as_posix())
...
class RaceControlsComponent(BaseComponent):
    ...
    def __init__(self, center_x: int = 100, center_y: int = 60, button_size: int = 40):
        ...
        _controls_folder = IMAGES_FOLDER / "controls"
        if _controls_folder.is_dir():
            for child in _controls_folder.iterdir():
                if child.suffix in (".png", ".jpg", ".jpeg"):
                    self._control_textures[child.stem] = arcade.load_texture(child.as_posix())
    ...
    def draw(self, window):
        ...
        self._draw_speed_comp(
            forward_x + self.speed_container_offset, self.center_y, getattr(window, "playback_speed", 1.0)
        )
    ...
    def _draw_speed_comp(self, x: float, y: float, speed: float):
        ...
        if "speed+" and "speed-" in self._control_textures:
            ...
            self.draw_hover_effect(
                "speed_increase", rect_plus.center_x, rect_plus.center_y, radius_offset=1, border_width=2
            )
            self.draw_hover_effect(
                "speed_decrease", rect_minus.center_x, rect_minus.center_y, radius_offset=1, border_width=2
            )
