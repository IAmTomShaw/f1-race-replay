...
class F1RaceReplayWindow(arcade.Window):
    ...
    def _broadcast_telemetry_state(self):
        ...
        tyre_health = {}
        if self.degradation_integrator and current_frame and "drivers" in current_frame:
            for code in current_frame["drivers"].keys():
                health = self.degradation_integrator.get_health_for_frame(code, current_frame)
                if health:
                    tyre_health[code] = {k: self._to_json_safe(v) for k, v in health.items()}
    ...
    def _to_json_safe(self, value):
        """Convert model outputs to JSON-safe scalar/list/dict values."""
