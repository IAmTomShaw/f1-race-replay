...
from f1_race_replay.f1_data import FPS, get_driver_quali_telemetry
from f1_race_replay.lib.time import format_time
from f1_race_replay.ui_components import (
        ...
        if self.chart_active and self.loaded_telemetry:
            ...
            if frames:
                fastest_driver = (
                    ...
                    if isinstance(self.data.get("results", []), list) and len(self.data.get("results", [])) > 0
                    else None
                )
                # Get comparison telemetry if available
                comparison_telemetry = (
                    self.data.get("telemetry", {}).get(fastest_driver.get("code")).get("Q3").get("frames", [])
                    if self.show_comparison_telemetry
                    and fastest_driver
                    and (
                        (fastest_driver.get("code") != self.loaded_driver_code)
                        or (
                            fastest_driver.get("code") == self.loaded_driver_code and self.loaded_driver_segment != "Q3"
                        )
                    )
                    else None
                )
                ...
                current_tel = (
                    current_frame.get("telemetry", {}) if isinstance(current_frame.get("telemetry", {}), dict) else {}
                )
                ...
                if draw_comparison_pos and draw_comparison_speeds:
                    ...
                    try:
                        ...
                        arcade.Text(
                            f"{current_speed:.0f} km/h", pts[-1][0] + 10, pts[-1][1] - 15, arcade.color.YELLOW, 12
                        ).draw()
                ...
                if getattr(self, "x_min", None) is not None and getattr(self, "x_max", None) is not None:
                    ...
                    if comparison_telemetry and self.frame_index < len(comparison_telemetry):
                        ...
                        comp_tel = (
                            comp_frame.get("telemetry", {}) if isinstance(comp_frame.get("telemetry", {}), dict) else {}
                        )
                        c_px = comp_tel.get("x")
                        c_py = comp_tel.get("y")
                ...
                if icon_keys:
                    ...
                    for key in icon_keys:
                        ...
                        if icon_texture:
                            ...
                            arcade.draw_texture_rect(rect=rect, texture=icon_texture, angle=0, alpha=255)
...
def run_qualifying_replay(session, data, title="Qualifying Results"):
    _ = QualifyingReplay(session=session, data=data, title=title)
    arcade.run()
