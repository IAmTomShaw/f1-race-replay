...
from f1_race_replay.f1_data import FPS
...
class F1RaceReplayWindow(arcade.Window):
    def __init__(
        ...
        self.race_controls_comp = RaceControlsComponent(center_x=self.width // 2, center_y=100)

        # Extract race events for the progress bar
        race_events = extract_race_events(frames, track_statuses, total_laps or 0)
        self.progress_bar_comp.set_race_data(total_frames=len(frames), total_laps=total_laps or 0, events=race_events)
    ...
    def on_draw(self):
        ...
        # Draw HUD - Top Left
        arcade.Text(lap_str, 20, self.height - 40, arcade.color.WHITE, 24, anchor_y="top").draw()
